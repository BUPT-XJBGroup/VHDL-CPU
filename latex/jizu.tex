...
\begin{figure}[!ht]
	...
	\begin{tikzpicture}
		...
		\draw [-] (-6, 0) -- (-6, 1.6);
		\draw [-] (-7, 0) -- (-7, 1.6);
		\draw [->] (-6.5, -0.5) -- (-6.5, -0.2);
		\node [below] at (-6.5, -0.5) {SP};
...
\indent 执行LSP指令以后，栈操作PUSHS，POPS有效，可用寄存器个数减少为3个。\\
\indent 执行SSP指令以后，存储器中的SP指针被更新，可用寄存器个数增加至4个。\\
\indent 新增栈以后，允许程序有更强大的功能，例如函数传参，天然拥有上下文无关文法的分析和编程功能等。
\newpage
\section{指令格式}
\begin{table}[htbp!]
    \centering
    \begin{tabular}{p{2.5cm}<{\centering}|p{2cm}<{\centering}|p{4cm}<{\centering}|p{2cm}<{\centering}|p{2cm}<{\centering}|p{2cm}<{\centering}}
        ...
        \cline{4-6}~ & ~ & ~ & IR7 $\sim$4 &IR3 IR2 & IR1 IR0\\
        \hline
        ...
        \cline{5-6}C条件转移 & JC addr & $PC \leftarrow @ + C\cdot \text{ofs}$ & 0111 & \multicolumn{2}{c}{ofs}\\
        \cline{5-6}Z条件转移 & JZ addr & $PC \leftarrow @ + Z\cdot \text{ofs}$ & 1000 & \multicolumn{2}{c}{ofs}\\
		\cline{5-6}无条件转移 & JMP [$R_d$] & $PC \rightarrow R_d$ & 1001 & $R_d$& XX\\
		输出 & OUT $R_s$ & $DBUS \leftarrow R_s$ & 1010 & XX & $R_s$\\
		\cline{5-6}取栈指针 & SSP $R_3$ & $R_3 \leftarrow [\text{0xFF}/SP]$ & 1011 & \multicolumn{2}{c}{1111}\\
		\cline{5-6}压栈 & PUSH $R_s$ & $[\text{0xFF}/SP] \leftarrow R_s $ & 1100 & 11 & $R_s$\\
		\cline{5-6}移动 & MOV $R_d$ $R_s$ & $R_d \leftarrow R_s$ & 1101 & $R_d$ & $R_s$\\
		停机 & STP & STOP& 1110 & XX& XX\\
		\cline{5-6}存栈指针 & LSP & $[\text{0xFF}/SP] \rightarrow R_s $ & 1111 & \multicolumn{2}{c}{1111} \\
